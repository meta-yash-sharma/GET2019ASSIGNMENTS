...
//				return printDir(currentDir);
	...
	private String mkdir(String command) {
		//split folder name 
		String folderName = command.substring((command.indexOf(" ") + 1), 
				(command.length() - 1));
		//create new node
	...
	private String ls() {
		for (int i = 0; i < currentDir.childList.size(); i++) {
			...
			System.out.println(currentDir.childList.get(i).getName() 
					+ "\t" + currentDir.childList.get(i).getDate());
			for (int j = 0; j < currentDir.childList.get(i).childList.size(); j++)
				System.out.println("\u2500" + currentDir.childList.get(i).childList.get(j).getName() 
						+ "\t" + currentDir.childList.get(i).childList.get(j).getDate());
		}
