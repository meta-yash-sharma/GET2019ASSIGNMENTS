...
SELECt product.product_id, product.product_name, product.product_price, product.stock, product.catagory_id
FROM product
...
FROM product_category c1 LEFT JOIN product_category c2
...
-- Display Product Title, Price & Description which falls into particular category Title (not working)--------------------
SELECT p.product_name,p.product_price,p.stock
FROM product p, product_category c
WHERE (c.category_id = p.catagory_id OR c.parent_id = p.catagory_id) AND c.category_name = 'mobile';
